...
wss.on('connection', (ws) => {
    ...
    serverBroadcastPlayerJoined(id);
...
// функция отправки сообщения каждому подключенному клиенту
...
function serverBroadcastPlayerJoined(clnt) {
    wss.clients.forEach((client) => {
        //client.send("JOINED ID TEST " + clnt)
        //client.send("JOINED ID TEEE " +)
        //Отправляется всем, кроме подключившемуся игроку
        if (clients.get(client) != clnt) {
            if (client.readyState === WebSocket.OPEN) {
                client.send("JOINED "+clnt);
            }
        }
    });
}
